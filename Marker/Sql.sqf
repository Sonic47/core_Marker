...
[{
	{
			...
			if (!(_grp getVariable "TF47_core_Marker_Sql_Freq_Intern" isEqualTo "")) then{
				_freq = _grp getVariable "TF47_core_Marker_Sql_Freq_Intern";
				_mkrName setMarkerTextlocal format[" %1 | %2 | %3", (groupId _grp), _leader, _freq];			
			}else{
				_mkrName setMarkerTextlocal format[" %1 | %2", (groupId _grp), _leader];
			};
...
private _actionsetFreq = ["setFreq", "Set Group Freq", "", {createDialog "tf47_core_marker_sql_set_freq"}, {player == leader group player}] call ace_interact_menu_fnc_createAction;
[player, 1, ["ACE_SelfActions","ACE_TeamManagement"], _actionsetFreq] call ace_interact_menu_fnc_addActionToObject;
